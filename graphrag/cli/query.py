...
def run_causal_search(
    ...
    """Perform a causal search with a given query.

    Loads index files required for causal search and calls the Query API.
    """
    root = root_dir.resolve()
    cli_overrides = {}
    if data_dir:
    ...
    if dataframe_dict["multi-index"]:
        ...
        logger.info(
            "Running multi-index causal search on indexes: %s",
            dataframe_dict["index_names"],
        )
        response, context_data = asyncio.run(
    ...
    # log the full response at INFO level for user visibility but at DEBUG level in the API layer
    logger.info("Causal Search Response:\n%s", response)
